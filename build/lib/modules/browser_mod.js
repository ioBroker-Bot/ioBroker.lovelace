...
class BrowserModModule {
  ...
  async _checkSettingState(ioBrokerDeviceId, browserId, key, name) {
    ...
    if (settingState && settingState.val !== null && settingState.val !== void 0) {
      if (browserId) {
   ...
   */
  _generateBrowserId() {
    const part = () => Math.floor(1e5 * (1 + Math.random())).toString(16).substring(1);
  ...
  async processMessage(ws, message) {
    ...
    if (message.type && message.type.startsWith("browser_mod/")) {
      ...
      if (message.browserID !== void 0 && typeof message.browserID !== "string") {
        this.adapter.log.warn(
      ...
      if (method === "update") {
        ...
        if (rawBrowserId !== void 0 && rawBrowserId !== message.browserID) {
          ...
          this._sendToClient(this.clients[message.browserID], {
            type: "event",
            event: {
              event_type: "browser_mod/command",
   ...
   */
  async _listOwnInstanceStateIds() {
    ...
    try {
      ...
      const ids = ((view == null ? void 0 : view.rows) || []).map((row) => {
        var _a;
        return row.id || ((_a = row.value) == null ? void 0 : _a._id);
      }).filter(Boolean);
      if (ids.length) {
        return ids;
      }
